...
# if network and subnet are in different project, then use fully qualified path e.g. 'projects/<sharedProject>/global/networks/<subnet|network>' instead of just name as done here
if [ "$lb_type" = "int" ]; then
  ...
  echo  "You can reach ext LB at: $IP"
fi
